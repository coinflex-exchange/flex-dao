...
def __init__(_lp_token: address, _minter: address, _admin: address):
  ...
  self.name = concat('CoinFLEX ', symbol, ' Gauge Deposit')
  self.symbol = concat(symbol, '-gauge')
  dao_token: address = Minter(_minter).token()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
  '''
  @notice Calculate limits which depend on the amount of DAO token per-user.
      Effectively it calculates working balances to apply amplification
      of DAO production by DAO
...
def set_killed(_is_killed: bool):
  ...
  @dev When killed, the gauge always yields a rate of 0 and so cannot mint DAO Token
